...
public class TimetableModelJDBCImpl implements TimetableModelInt {
	...
	public long add(TimetableDTO tdto) throws ApplicationException {
		...
		try {
			...
			ps.setLong(1, pk);
			ps.setLong(2, tdto.getSubjectId());
			ps.setString(3, tdto.getCourseName());
			ps.setDate(4, date);
			ps.setString(5, tdto.getSubjectName());
	...
	public void update(TimetableDTO tdto) throws DatabaseException, ApplicationException {
		...
		try {
			...
			ps.setLong(1, tdto.getSubjectId());
			ps.setString(2, tdto.getSubjectName());
	...
	public List search(TimetableDTO tdto1, int pageNo, int pageSize) throws ApplicationException {
		...
		if (tdto1 != null) {
			...
			if (tdto1.getSubjectId() > 0) {
				sql.append(" AND SUB_ID = " + tdto1.getSubjectId());
			}
			if ((tdto1.getSubjectName() != null) && (tdto1.getSubjectName().length() > 0)) {
				sql.append(" AND SUB_NAME like '" + tdto1.getSubjectName() + "%'");
			}

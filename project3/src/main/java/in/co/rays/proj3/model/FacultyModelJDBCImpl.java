...
public class FacultyModelJDBCImpl implements FacultyModelInt {
	...
	public long add(FacultyDTO dto) throws ApplicationException, DuplicateRecordException {
		 ...
		 * CollegeDTO dto1 = model.findByPK(dto.getCollegeId()); String CollegeName =
		 * dto1.getName();
		 * 
		 * 
		 * CourseModelInt model1 = ModelFactory.getInstance().getCourseModel();
		 * CourseDTO dto2 = model1.findByPK(dto.getCourseId()); String CourseName =
		 * dto2.getCourseName();
		 * 
		 * SubjectModelInt model2 = ModelFactory.getInstance().getSubjectModel();
		 * SubjectDTO dto3 = model2.findByPK(dto.getSubjectId()); String SubjectName =
		 * dto3.getSubjectName();
		 * 
		 * FacultyDTO duplicataRole = findByEmailId(dto.getEmailId()); // Check if
		 * create Faculty already exist if (duplicataRole != null) { throw new

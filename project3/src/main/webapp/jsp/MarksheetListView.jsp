			...
			<div class="card"
				...
				<div class="card-body bg-dark">
					...
					<%
						...
						System.out.println("...............List in Marksheet List Ctl............." + list);
						if (list.size() == 0) {
					%>
					<table align="center">
						<tr>
					...
					<div class="row no-gutters">
						...
						<i style="color: white"><b>RollNo :</b></i>&nbsp;
						<%
							List roleList = (List) request.getAttribute("roleList");
						%>
						<div class="col-sm-2">
							<%=HTMLUtility2.getList("roll", String.valueOf(dto.getId()), rollList)%>
						</div>
						&emsp;&emsp; <i style="color: white"><b>Student Name :</b></i>&nbsp;
						<div class="col-sm-2">
							<input placeholder="Enter Student Name"
								style="background-color: white; max-width: 270px" type="text"
					...
					<div class="row no-gutters">
						<button type="submit" style="font-size: 13px" name="operation"
							value="<%=MarksheetListCtl.OP_NEW%>" class="btn btn-success">
							<span class="fas fa-plus-square"></span>
							<%=MarksheetListCtl.OP_NEW%>
						</button>
						<div class="ml-auto">
							<button type="submit" style="font-size: 13px" name="operation"
								value="<%=MarksheetListCtl.OP_DELETE%>" class="btn btn-danger">
					...
					<table class="table table-light table-striped" border="2">
						...
						<tbody>
							<tr align="center">
								...
								<td><strong><%=per%>%</td>
								<%
									int avg = dto.getPhysics() + dto.getChemistry() + dto.getMaths();
								%>
								<%
									if (avg >= 80 && !(dto.getPhysics() <= 32 || dto.getChemistry() <= 32 || dto.getMaths() <= 32)) {
												grade = "A+";
											} else if (l > 60 && avg <= 80
													&& !(dto.getPhysics() <= 32 || dto.getChemistry() <= 32 || dto.getMaths() <= 32)) {
												grade = "A";
